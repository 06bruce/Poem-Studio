...
// `avatar` is stored as an unbounded String, and the profile editor saves
// uploaded images as base64 data URIs directly into it — a photo can be
// 1-3MB as text. A list of poems populates one author per poem, so this
// alone can dwarf the comments/likes trimming below. Strip it in the
// aggregation itself so Mongo never sends it over the wire; the UI already
// falls back to an initial-letter avatar when it's unset.
function projectAuthorFields(authorProjection) {
  const { avatar, ...rest } = authorProjection;
  if (!avatar) return { $project: authorProjection };
  return {
    $project: {
      ...rest,
      avatar: {
  ...
  return [
    ...
    { $project: { likes: 0, comments: 0, annotations: 0 } },
    {
      $lookup: {
        ...
        foreignField: '_id',
        as: 'author',
        pipeline: [projectAuthorFields(authorProjection)],
      },
    },
